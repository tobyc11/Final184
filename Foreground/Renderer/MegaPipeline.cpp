    ...
    , VoxelizeRenderer(this)
{
    ...
    desc.MipmapMode = RHI::ESamplerMipmapMode::Linear;
    ...
    desc.AddressModeV = RHI::ESamplerAddressMode::Clamp;
    desc.AddressModeW = RHI::ESamplerAddressMode::Clamp;
    VoxelSampler = RenderDevice->CreateSampler(desc);
...
void CMegaPipeline::Render()
{
    ...
    auto voxelDS = pb.CreateDescriptorSet();
    pb.BindImageView(voxelDS, VoxelBuffer, "voxels");
    pb.BindImageView(voxelDS, ShadowDepth, "t_shadow");
    pb.BindConstants(voxelDS, &matricesConstants, sizeof(matricesConstants), "ExtendedMatrices");
    pb.BindConstants(voxelDS, &directionalLightLists, sizeof(LightLists), "directionalLights");
    pb.BindConstants(voxelDS, &pointLightLists, sizeof(LightLists), "pointLights");
    VoxelizeRenderer.SetVoxelDS(voxelDS);
    ...
    // Voxel volume: down filter
    copyCtx = cmdList->CreateCopyContext();
    RHI::CImageBlit blit;
    ...
    uint32_t currWidth = 256;
    for (uint32_t currLevel = 1; currLevel < 9; currLevel++)
    {
        blit.SrcOffsets[1].Set(currWidth, currWidth, currWidth);
        if (currWidth > 1)
            currWidth /= 2;
        blit.DstOffsets[1].Set(currWidth, currWidth, currWidth);
        blit.SrcSubresource.MipLevel = currLevel - 1;
        blit.DstSubresource.MipLevel = currLevel;
        copyCtx->BlitImage(*VoxelImage, *VoxelImage, { blit }, RHI::EFilter::Linear);
    }
    copyCtx->FinishRecording();

    // auto computeCtx = cmdList->CreateComputeContext();
    // uint32_t currWidth = 128;
    // for (uint32_t currLevel = 1; currLevel < 9; currLevel++)
    // {
    //     VoxelDownfilter->SetImageView("voxels", VoxelLod[currLevel - 1]);
    //     VoxelDownfilter->SetImageView("outVoxels", VoxelLod[currLevel]);
    //     VoxelDownfilter->Dispatch(*computeCtx, currWidth, currWidth, currWidth);
    //     currWidth /= 2;
    //}
    // computeCtx->FinishRecording();

    lighting_indirect->beginRender(cmdList);
    lighting_indirect->setSampler("s", VoxelSampler);
    ...
    lighting_indirect->setImageView("t_material", GBuffer2);
    ...
    gtao_color->setStruct("EngineCommonMiscs", sizeof(EngineCommonMiscs), &miscs);
    gtao_color->blit2d();
...
void CMegaPipeline::BindEngineCommonForView(RHI::IRenderContext& context, uint32_t viewIndex)
{
    ...
    pb.BindConstants(EngineCommonDS, &miscs, sizeof(EngineCommonMiscs), "EngineCommonMiscs");
...
void CMegaPipeline::CreateShadowBufferPass()
{
    ...
    auto depthImage = RenderDevice->CreateImage2D(
        EFormat::D32_SFLOAT, EImageUsageFlags::DepthStencil | EImageUsageFlags::Sampled, 2048,
        2048);

    CImageViewDesc depthViewDesc;
    depthViewDesc.Format = EFormat::D32_SFLOAT;
...
void CMegaPipeline::CreateVoxelizePass()
{
    ...
    const uint32_t voxelSize = 256;
    VoxelImage = RenderDevice->CreateImage3D(EFormat::R16G16B16A16_SFLOAT,
                                             EImageUsageFlags::Storage | EImageUsageFlags::Sampled,
                                             voxelSize, voxelSize, voxelSize, 9);

    CImageViewDesc voxelViewDesc;
    voxelViewDesc.Format = EFormat::R16G16B16A16_SFLOAT;
    voxelViewDesc.Type = EImageViewType::View3D;
    voxelViewDesc.Range.Set(0, 9, 0, 1);
    VoxelBuffer = RenderDevice->CreateImageView(voxelViewDesc, VoxelImage);

    for (uint32_t i = 0; i < 9; i++)
    {
        voxelViewDesc.Range.Set(i, 1, 0, 1);
        VoxelLod[i] = RenderDevice->CreateImageView(voxelViewDesc, VoxelImage);
    }

    CRenderPassDesc rpDesc;
    rpDesc.NextSubpass().ColorAttachments.clear();
    rpDesc.Width = voxelSize;
    rpDesc.Height = voxelSize;
    rpDesc.Layers = 1;
    VoxelizationPass = RenderDevice->CreateRenderPass(rpDesc);

    VoxelDownfilter = std::make_unique<CComputePass>("voxel_mipmap.comp.spv");
}
